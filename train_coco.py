...
def get_arguments():
    ...
    parser.add_argument("-Dcoco", type=str, help="path to coco",default='data/coco2017/')
    ...
    parser.add_argument("-total_iter", type=int, help="total iter num",default=600000)
...
for iter_ in range(args.total_iter):
	...
	try:
		Fs, Ms, num_objects, info = next(loader_iter1)  #Fs[1,3,3,384,384] Ms[1,11,3,384,384]
		Fs = Fs.cuda()
		Ms = Ms.cuda()
	except:
		loader_iter1 = iter(Trainloader1)
	...
	n2_logit = model(Fs[:,:,1], n1_key, n1_value, torch.tensor([num_objects]))  # 1,11,384,384
	...
	if (iter_+1) % save_step == 0 and (iter_+1) >= 400000:
