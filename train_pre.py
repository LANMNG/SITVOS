...
from dataset.PreTrain_DS import PreTrain_DS
...
def get_arguments():
    ...
    parser.add_argument("-Ddavis", type=str, help="path to davis",default='data/DAVIS/')
    parser.add_argument("-Dpretrain", type=str, help="path to coco",default='data/pretrain')
    ...
    parser.add_argument("-total_iter", type=int, help="total iter num",default=700000)
...
if args.wandb:
	...
	wandb.run.name = 'pretrain_weights'

DAVIS_ROOT = args.Ddavis
Pretrain_ROOT = args.Dpretrain
# palette = Image.open(DAVIS_ROOT + 'Annotations/480p/blackswan/00000.png').getpalette()

torch.backends.cudnn.benchmark = True

Trainset1 = PreTrain_DS(Pretrain_ROOT, 384)
Trainloader1 = data.DataLoader(Trainset1, batch_size=1, num_workers=2, shuffle = True, pin_memory=True)
...
for iter_ in range(args.total_iter):
	...
	try:
		...
		num_oj = num_objects.item()
		if num_oj == 0:
			print("num_object is 0")
			Fs, Ms, num_objects, info = next(loader_iter1)
		Fs = Fs.cuda()
		Ms = Ms.cuda()
	except:
	...
	n2_logit, _ = model(Fs[:,:,1], n1_key, n1_value, torch.tensor([num_objects]))  # 1,11,384,384

	n2_label = torch.argmax(Ms[:,:,1],dim = 1).long().cuda() # 1,384,384
	...
	if (iter_+1) % log_iter == 0:
		...
		logger.info('v1: iteration:{}, loss:{}, remaining iteration:{}, lr:{}'.format(iter_, loss_log, args.total_iter - iter_, lr_iter))
		loss_momentum = 0
		if args.wandb:
